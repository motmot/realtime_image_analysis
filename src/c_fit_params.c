/* $Id: c_fit_params.c 1614 2007-02-28 01:17:56Z astraw $ */
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include "c_fit_params.h"
#include "moments.h"
#include <fwBase.h>

#define CHK( a ) (a == fwStsNoErr? 1:0)

/****************************************************************
** fit_params ***************************************************
****************************************************************/
CFitParamsReturnType fit_params( cFitParamsMomentState_64f *pState,
                                 double *x0, double *y0,
                                 double *Mu00,
                                 double *Uu11, double *Uu20, double *Uu02,
                                 int width, int height,
                                 unsigned char *img, int img_step )
{
  double Mu10, Mu01;
  FwiSize roi_size;
  FwiPoint roi_offset;

  roi_offset.x = 0;
  roi_offset.y = 0;
  roi_size.width = width;
  roi_size.height = height;

  print_state(pState);

  /* get moments */
  if( !CHK( cFitParamsMoments64f_8u_C1R( (Fw8u*)img, img_step, roi_size, pState ) ) )
  {
    return CFitParamsOtherError;
  }

  /* calculate center of gravity from spatial moments */
  if( !CHK( cFitParamsGetSpatialMoment_64f( pState, 0, 0, 0, roi_offset, (Fw64f*)Mu00 ) ) )
  {
    return CFitParamsOtherError;
  }
  if( !CHK( cFitParamsGetSpatialMoment_64f( pState, 1, 0, 0, roi_offset, (Fw64f*)&Mu10 ) ) )
  {
    return CFitParamsOtherError;
  }
  if( !CHK( cFitParamsGetSpatialMoment_64f( pState, 0, 1, 0, roi_offset, (Fw64f*)&Mu01 ) ) )
  {
    return CFitParamsOtherError;
  }
  if( *Mu00 != 0.f )
  {
    *x0 = Mu10 / *Mu00;
    *y0 = Mu01 / *Mu00;
    /* relative to ROI origin */
  }
  else
  {
    return CFitParamsZeroMomentError;
  }

  /* calculate blob orientation from central moments */
  if( !CHK( cFitParamsGetCentralMoment_64f( pState, 1, 1, 0, (Fw64f*)Uu11 ) ) )
  {
    return CFitParamsCentralMomentError;
  }
  if( !CHK( cFitParamsGetCentralMoment_64f( pState, 2, 0, 0, (Fw64f*)Uu20 ) ) )
  {
    return CFitParamsCentralMomentError;
  }
  if( !CHK( cFitParamsGetCentralMoment_64f( pState, 0, 2, 0, (Fw64f*)Uu02 ) ) )
  {
    return CFitParamsCentralMomentError;
  }

  return CFitParamsOtherError;
}
